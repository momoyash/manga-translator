...
Supports DeepL, GPT-4o, Gemini, DeepSeek, Groq, and offline models.

Usage:
  python run.py -i "path/to/manga" -o "path/to/output"
...
from mtl.renderer import fix_folder, clip_overflow

# ── Paths ──────────────────────────────────────────────────────────────────────
ROOT     = Path(__file__).parent
ENGINE   = ROOT / "tl-core" if (ROOT / "tl-core").exists() else ROOT.parent / "manga-image-translator"
...
def run(input_path: str, output_path: str, translator: str = None,
    ...
    src = Path(input_path)
    dst = Path(output_path).resolve()  # Use absolute path
    ...
    cmd = [
        ...
        "--use-gpu" if gpu else "",
        "local",
        "-i", str(src.resolve()),  # Use absolute path
        "-o", str(dst),
        "-f", fmt,
        "--overwrite",
    ...
    # Fallback: copy from engine's result folder if output is empty
    engine_result = ENGINE / "result" / "final.png"
    if result.returncode == 0:
        if engine_result.exists():
            ...
            if not out_path.exists() or out_path.stat().st_mtime < engine_result.stat().st_mtime:
                shutil.copy2(engine_result, out_path)
                print(f"  [copied] {engine_result.name} -> {out_path.name}")
