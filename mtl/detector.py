...
INPUT_SIZE  = 640
MIN_SCORE   = 0.25  # Lowered to catch more small bubbles
...
def scan_cv(img: np.ndarray, debug: bool = False) -> List[Region]:
    ...
    for i, c in enumerate(contours):
        ...
        # Allow smaller bubbles (down to 0.0005 of page area)
        if not (page_area * 0.0005 < area < page_area * 0.30):
            continue
        x, y, bw, bh = cv2.boundingRect(c)
        if bh == 0 or not (0.15 < bw / bh < 6.0):  # More permissive aspect ratio
            continue
        hull_area = cv2.contourArea(cv2.convexHull(c))
        if hull_area == 0 or area / hull_area < 0.45:  # Slightly more permissive
            continue
        if bw * bh == 0 or area / (bw * bh) < 0.25:  # More permissive fill ratio
            continue
        mask = np.zeros((h, w), np.uint8)
        cv2.drawContours(mask, [c], -1, 255, -1)
        brightness = cv2.mean(gray, mask=mask)[0]
        # Lower brightness threshold to catch gray/off-white bubbles
        if brightness < 150:
            continue
...
def _find_contour_in_region(img: np.ndarray, pts: np.ndarray) -> Optional[np.ndarray]:
    """
    Find the largest contour within a bounding box region.
    Used to get accurate bubble shapes for model-detected regions.
    """
    h, w = img.shape[:2]
    ...
    # Try to find bright bubble region
    ...
    # Offset contour back to original coordinates
    largest = largest + np.array([x1, y1])
    return largest


def refine_with_contours(img: np.ndarray, regions: List[Region]) -> List[Region]:
    """Add accurate contours to model-detected regions."""
    for r in regions:
        if r.method == "model" and r.contour is None:
            contour = _find_contour_in_region(img, r.pts)
...
def find(img: np.ndarray, min_score: float = MIN_SCORE,
         debug: bool = False, refine: bool = True) -> List[Region]:
    """
    ...
    Args:
        ...
        refine: Try to find accurate contours for model-detected regions
    """
    ...
    # Try to add accurate contours to model detections
    if refine:
        # First check if any CV regions overlap with deep regions and use their contours
        for d in deep:
            d_box = _box(d.pts)
            for c in cv:
                if _iou(d_box, _box(c.pts)) > 0.5 and c.contour is not None:
                    d.contour = c.contour
                    break
        # For remaining, try to extract contours
        deep = refine_with_contours(img, deep)
