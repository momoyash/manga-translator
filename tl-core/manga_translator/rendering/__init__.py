    ...
    for region in text_regions: 
        ...
        if region.horizontal: 
            ...
            if needed_rows > used_rows:
                MAX_EXPANSION = 2.0
                natural_scale_x = ((needed_rows - used_rows) / used_rows) + 1
                scale_x = min(natural_scale_x, MAX_EXPANSION)
                # If we had to cap the expansion, reduce font size proportionally so text fits
                if natural_scale_x > MAX_EXPANSION:
                    target_font_size = max(
                        int(target_font_size * (MAX_EXPANSION / natural_scale_x)),
                        font_size_minimum
                    )
                try:
        ...
        if region.vertical:
            ...
            if needed_cols > used_cols:
                MAX_EXPANSION = 2.0
                natural_scale_x = ((needed_cols - used_cols) / used_cols) + 1
                scale_x = min(natural_scale_x, MAX_EXPANSION)
                if natural_scale_x > MAX_EXPANSION:
                    target_font_size = max(
                        int(target_font_size * (MAX_EXPANSION / natural_scale_x)),
...
async def dispatch_eng_render_pillow(img_canvas: np.ndarray, original_img: np.ndarray, text_regions: List[TextBlock], font_path: str = '', line_spacing: int = 0, disable_font_border: bool = False, font_size_offset: int = 0) -> np.ndarray:
    if len(text_regions) == 0:
        return img_canvas
    ...
    # Apply font_size_offset before rendering
    for region in text_regions:
        region.font_size = max(1, region.font_size + font_size_offset)

    return render_textblock_list_eng_pillow(font_path, img_canvas, text_regions, original_img=original_img, downscale_constraint=0.65)
